...
const NotificationCard = ({ actor, story_id, created_at, notification_type }) => {
    ...
    return (
        <div className="flex items-center gap-3 lg:gap-4 py-2">
            <Link to={profileLink} className="block self-start w-10 lg:w-12 aspect-square shrink-0"><img src={actor.image ? `${MEDIA_URL}${actor.image}` : "/images/default_avatar.jpg"} alt="profile picture" className="w-full h-full rounded-full" /></Link>
